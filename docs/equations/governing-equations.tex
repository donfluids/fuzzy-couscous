...
\paragraph{Shock capturing for the five-equation system.}
All five equations --- conservative momentum and energy, the two conservative
partial masses (with $\rho=Z_1+Z_2$ derived), and the non-conservative volume
fraction $\alpha_1$ --- are advanced through the \emph{same} shock-capturing
engine: one Ducros sensor $\theta$ (\S\ref{sec:lad}, dilated $\pm2$ cells along
the face direction), one hybrid reconstruction that uses 6th-order central
differencing where $\theta\le0.65$ and 5th-order WENO where $\theta>0.65$, and one
Lax--Friedrichs split with a \emph{single shared} wave speed
$\alpha=|u_d|+c$ ($c$ the frozen mixture sound speed). Because every conserved and
non-conserved quantity sees the same $\alpha$ and the same sensor, the contact
remains in pressure/velocity equilibrium. Unlike the two-$\gamma$ model
(\S\ref{sec:gammatransport}) the five-equation model therefore needs \emph{no}
double-flux: the stiffened-gas mixture closure makes the closure coefficients
$S$ and $\Pi$ \emph{linear} in $\alpha_1$ (\S\ref{sec:eos5eq}), so the
volume-fraction-averaged pressure $p=(\rho e-\Pi)/S$ stays uniform across a
uniform-$(p,\uvec)$ interface --- the sensor sees no jump, WENO is not triggered,
and the plain conservative flux is already oscillation-free
(\code{numerics/RHS.cpp}, \code{fill\_flux\_alpha\_5eq} /
\code{add\_inviscid\_divergence\_5eq}).
...
\subsection{Artificial diffusivity for the five-equation interface}
\label{sec:lad5eq}
The base LAD above acts on the conserved state only. For the five-equation model
(\S\ref{sec:fiveeq}) it is extended to the aux fields so the material interface is
regularized \emph{without} breaking the contact's pressure/velocity equilibrium
(\code{numerics/ArtificialDiffusivity.cpp}, \code{compute\_lad\_fields\_5eq};
\code{numerics/RHS.cpp}, \code{add\_rhs\_viscous\_5eq}). Two changes are required.
First, the contact diffusivity is re-keyed on the \emph{interface} markers rather
than the mixture density (which can be smooth where $\alpha_1$ jumps),
\begin{equation}
D^\ast = C_D\,c_{\mathrm{mix}}\,
  \max\!\Big(|\nabla^r\alpha_1|,\ \tfrac{|\nabla^r Z_1|}{Z_1},\ \tfrac{|\nabla^r Z_2|}{Z_2}\Big)\,\Delta,
\end{equation}
with $c_{\mathrm{mix}}$ the mixture sound speed (consistent with the inviscid
wave speed $\alpha=|u_d|+c$). Second, the thermal term is disabled
($\kappa^\ast=0$): the cell-gradient temperature is an ideal-gas reduction that is
\emph{non-uniform} across a uniform-pressure contact, so a thermal flux keyed on
it would inject spurious pressure; the bulk/shear terms are retained (they vanish
at a stationary contact).

The interface is then regularized by a consistent diffusion~\cite{cook_2009}: the
two partial masses, the volume fraction, \emph{and} the internal-energy density
$\rho e_{\mathrm{int}}=\rho E-\tfrac12\rho|\uvec|^2$ all diffuse with the
\emph{same} coefficient and stencil,
\begin{equation}
\partial_t q \mathrel{+}= \nabla\!\cdot(D^\ast\nabla q),
\qquad q\in\{Z_1,\,Z_2,\,\alpha_1,\,\rho e_{\mathrm{int}}\},
\end{equation}
with the mixture-mass flux taken as the sum of the partial-mass fluxes (so
$\rho=Z_1+Z_2$ is preserved) and momentum carried at the local velocity. Because
the stiffened-gas closure makes $\rho e_{\mathrm{int}}=p\,S(\alpha_1)+\Pi(\alpha_1)$
\emph{affine} in $\alpha_1$ (\S\ref{sec:eos5eq}), applying the same linear operator
to $\alpha_1$ and to $\rho e_{\mathrm{int}}$ leaves the state on the manifold
$p=(\rho e_{\mathrm{int}}-\Pi)/S$ at the \emph{unchanged} $p$: a uniform-$(p,\uvec)$
interface stays in pressure equilibrium to round-off (exact for stiffened-gas
phases; approximate for JWL phases, where $\Pi$ is non-linear in the phase
density). This is verified end-to-end by \code{tests/test\_five\_eq\_abv.cpp}
(stationary contact: $p$ uniform to $10^{-9}$ while $\alpha_1$ spreads; shock
crossing an interface with LAD active stays bounded and positive).

\subsection{Robustness: closely-spaced shocks and positivity}
\label{sec:robustness}
WENO5-JS~\cite{jiang_shu_1996} assumes \emph{at most one} discontinuity inside its
five-point stencil, so that at least one of its three candidate sub-stencils is
smooth. Two shocks that approach within roughly a stencil width violate this
premise: all three smoothness indicators $\beta_0,\beta_1,\beta_2$ become large and
comparable, the nonlinear weights can no longer isolate a smooth sub-stencil, and
the scheme loses both its design order and its non-oscillatory property. In the
worst case a strong expansion trapped between the two shocks drives the
reconstructed density or internal energy negative, which makes the sound speed
$c=\sqrt{\gamma p/\rho}$ undefined and poisons the field. The reconstruction here
is plain WENO5-JS rather than the more robust WENO-Z~\cite{borges_2008} or mapped
WENO~\cite{henrick_2005}, so this regime is guarded by several explicit
mechanisms rather than by the reconstruction alone:
\begin{enumerate}
\item \textbf{Sensor dilation.} The $\pm2$-cell max-dilation of $\theta$
  (\S\ref{sec:lad}) forces WENO over the \emph{entire} region spanning both shocks,
  so the central scheme never rings in the gap between them.
\item \textbf{Lax--Friedrichs dissipation.} The shocked-face flux carries local LF
  dissipation with $\alpha=|u_d|+c$, taken as the max over the two adjacent cells;
  LF is the most dissipative (most robust) split and damps the inter-shock
  oscillations the reconstruction may generate.
\item \textbf{Sound-speed guards.} Every $c=\sqrt{\cdot}$ is taken as
  $\sqrt{\max(\cdot,0)}$ (\code{physics/MixtureEOS.hpp}), so a transient negative
  $c^2$ cannot inject a NaN before the floors below act.
\item \textbf{Per-stage positivity floors.} After \emph{each} of the three SSP-RK3
  stages the state is repaired before the next sound-speed evaluation:
  \code{enforce\_positivity} floors $\rho$ and the internal energy (guaranteeing
  $p>0$ independent of the local $\gamma$, \code{core/State.hpp}), and for the
  five-equation model \code{enforce\_5eq\_bounds} clamps
  $\alpha_1\in[\alpha_{\mathrm{floor}},1-\alpha_{\mathrm{floor}}]$, keeps
  $Z_1,Z_2\ge z_{\mathrm{floor}}$, and resets $\rho=Z_1+Z_2$
  (\code{physics/Multifluid.cpp}). The floors are auto-scaled to the ambient state
  so resolved physics is untouched; the per-stage clamp count is reported as a
  diagnostic.
\item \textbf{LAD backstop.} The localized artificial diffusivity above is an
  opt-in, tunable dissipation sink that targets exactly the shock and contact
  cells (with a \code{disable\_weno} ``LAD-only'' mode); its diffusivity also
  enters the viscous time-step limit, so added robustness stays time-stable.
\end{enumerate}
These are floors and added dissipation, not a provably positivity-preserving
limiter~\cite{zhang_shu_2010}; the reconstruction is also component-wise rather
than characteristic-wise. In practice the combination keeps shock--shock and
shock--contact interactions bounded (exercised by the Shu--Osher and
five-equation shock-bubble tests), at the cost of extra local dissipation where
shocks crowd together.
...
(\code{numerics/RK3.cpp}, \code{numerics/RHS.cpp}). Each of the three stages is
followed by positivity/bounds enforcement (\code{enforce\_positivity}, and for the
five-equation model \code{enforce\_5eq\_bounds}); this per-stage repair is the
primary robustness backstop for strong or closely-spaced shocks
...
\bibitem{henrick_2005}
A.~K. Henrick, T.~D. Aslam, and J.~M. Powers,
``Mapped weighted essentially non-oscillatory schemes: achieving optimal order
near critical points,''
\textit{J.\ Comput.\ Phys.} \textbf{207}(2):542--567, 2005.
