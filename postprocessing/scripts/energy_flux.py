...
def case_runs(case):
    ...
    if case == "128":
        runs = [(f"out_blast_128_budget_seed{s}", f"blast_128_budget_seed{s}")
                for s in (1, 2, 3, 4, 5)]
        spec = run_dir("out_blast_128_budget_seed1") / "blast_128_budget_seed1_spectra.h5"
    elif case == "256":
        runs = [("out_blast_256_match128", "blast_256_match128")]
        spec = run_dir("out_blast_256_match128") / "blast_256_match128_spectra.h5"
    else:
...
def snaps(d, name):
    out = []
    for p in sorted(glob.glob(str(run_dir(d) / f"{name}_*.h5"))):
