#!/usr/bin/env bash
# Configure the MPI build (build_mpi/) using apt OpenMPI 4.1.6 + apt FFTW3,
# overriding any sideloaded /usr/local FFTW or /opt/openmpi-5 that cmake's
# default search would otherwise pick up.
#
# Usage (from the repo root):
#   ./solver/cmake/configure-mpi.sh                  # configures build_mpi/
...
PATH=/usr/bin:/bin cmake -S solver -B build_mpi \
