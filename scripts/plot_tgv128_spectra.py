#!/usr/bin/env python3
"""TGV 128^3 spectra comparison: nabla^4 FD baseline vs nabla^6 FD vs nabla^6 spectral.

Reads the spectra HDF5 files written by blast_les / blast_les_mpi, finds the
time of peak total dissipation in each, and plots E_total(k) on a log-log
axis with a k^{-5/3} reference line. Output: figs/tgv128_spectra.png.
"""
...
RUNS = [
    (
        r"$\nabla^4$ FD (baseline)",
        "out_tgv128_hyper2",
        "tgv_re1600_128_hyper2_spectra.h5",
        "tgv_re1600_128_hyper2_stats.csv",
        "tab:blue",
    ),
    (
        r"$\nabla^6$ FD",
        "out_tgv128_hyper6_fd",
        "tgv_re1600_128_hyper6_fd_spectra.h5",
        "tgv_re1600_128_hyper6_fd_stats.csv",
        "tab:orange",
    ),
    (
        r"$\nabla^6$ spectral",
        "out_tgv128_hyper6_spectral",
        "tgv_re1600_128_hyper6_spectral_spectra.h5",
        "tgv_re1600_128_hyper6_spectral_stats.csv",
        "tab:green",
    ),
]
...
def main():
    ...
    ap.add_argument("--out", default=str(ROOT / "figs" / "tgv128_spectra.png"))
    args = ap.parse_args()
    ...
    def plot_one(ax, t_chooser, title):
        ...
        if amp_anchor is None:
            amp_anchor = 1e-3
        kref = np.logspace(np.log10(2.0), np.log10(60.0), 60)
    ...
    fig.suptitle(r"TGV Re=1600, $128^3$: $E(k)$ comparison", y=1.02)
